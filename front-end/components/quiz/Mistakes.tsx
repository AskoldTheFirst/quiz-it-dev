...
import { loadMistakes, setIsByTotal, setTopicId } from "@/redux/mistakesSlice";
import { useEffect } from "react";

export default function Mistakes() {
    const { topics } = useSelector((state: RootState) => state.appState);
    const { mistakes, topicId, isByTotal, isLoaded } = useSelector((state: RootState) => state.mistakesState);
    const dispatch = useAppDispatch();

    useEffect(() => {
        dispatch(loadMistakes({ topicId: topicId, byTotal: isByTotal, topCount: 5 }));
    }, [dispatch, topicId, isByTotal]);
        ...
        <Box>
          ...
          Top 5 mistakes for {}
            ...
            <Card sx={{ backgroundColor: "#1e293b", border: "1px solid rgba(148, 163, 184, 0.1)", mb: 3 }}>
                <CardContent sx={{ p: 2.5, "&:last-child": { pb: 2.5 } }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
                        ...
                        >
                            <Typography variant="caption" sx={{ color: isByTotal ? "#10b981" : "#64748b", fontWeight: 700 }}>
                                Total
                            </Typography>
                            <Switch
                                size="small"
                                checked={!isByTotal}
                                onChange={(_, checked) => dispatch(setIsByTotal(!checked))}
                                sx={{
                            ...
                            <Typography
                                variant="caption"
                                sx={{ color: !isByTotal ? "#10b981" : "#64748b", fontWeight: 700 }}
                            >
                                By %
                            </Typography>
                        </Box>
            ...
            <Card sx={{ backgroundColor: "#1e293b", border: "1px solid rgba(148, 163, 184, 0.1)" }}>
                {!isLoaded && <LinearProgress sx={{ backgroundColor: "rgba(148,163,184,0.1)", "& .MuiLinearProgress-bar": { backgroundColor: "#10b981" } }} />}
                <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1.25 }}>
                        {mistakes.map((row) => {
                            return (
                                <Card
                                    key={`${row.questionText}-${row.totalCount}`}
                                    sx={{
                                        position: "relative",
                                ...
                                >
                                    ...
                                    >
                                        <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1, flex: 1, minWidth: 0 }}>
                                            <Typography variant="body2" sx={{ color: "#e2e8f0", lineHeight: 1.45 }}>
                                                {row.questionText}
                                            </Typography>
                                        </Box>
